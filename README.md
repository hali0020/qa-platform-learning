...
- [游戏 QA 测试方法与面试手册](docs/GAME_QA_TESTING_INTERVIEW_GUIDE.md)：按测试类型整理目标、流程、数据采集、计算公式、阈值口径、源码审计边界和常见面试答法。
- [独立 Pytest 学习网站测试包](pytest/README.md)：覆盖公开 API 操作面、核心业务旅程与横切质量属性，并生成离线指标可视化报告。
- [专项测试实现与指标数据汇总](docs/专项测试实现与指标数据汇总.docx)：汇总当前实现、实测数据、参考门槛和未完成边界。
...
Compose 拓扑包含一个不对宿主机暴露端口的项目内 Redis。当前只缓存项目列表与
项目详情，数据库始终是唯一事实来源：

1. 查询先读取 Redis；命中后直接反序列化返回。
2. 未命中、缓存内容损坏或 Redis 不可用时，读取数据库并尽力回填缓存。
3. 创建、更新、状态转换或删除项目时，先提交数据库，再删除相关列表和详情缓存。
4. 缓存默认 60 秒过期，Redis 使用 128 MiB 上限与 LRU 淘汰策略，不做持久化。

源码直接运行时默认禁用 Redis。Compose 使用固定的 redis://redis:6379/0，
可在私有 .env 中调整 TTL 和操作超时。

展示型列表和详情读取缓存；写入前的存在性、状态流转及关联数据校验始终读取原始
数据库仓库。Prometheus 暴露缓存命中/未命中/异常、回填/失效结果，以及数据库
回源次数和耗时；接口延迟继续由现有 HTTP Histogram 统计。
