...
- 需求覆盖/代码覆盖数据源、历史质量事实仓库和自动质量门禁。
- 独立 Worker/Scheduler/Agent 服务、消息代理和多实例领导者选举。
...
当前机器没有 Docker，PostgreSQL 尚未做真实容器启动、迁移、CRUD、流水线重启恢复和 readiness 联调；当前完成的是共享持久化实现、配置边界测试与 PostgreSQL 方言离线验证。整体仍是单进程教学拓扑。

## 怎样判断“阶段二完成”

这里的完成标准是：七大能力都有可读模型、稳定边界、HTTP 或本机演示入口、自动化测试和生产差距说明。它不是“所有企业功能都完成”。

下一阶段重点不是继续堆页面，而是 Unit of Work、项目级授权、真实 PostgreSQL 容器集成与数据搬迁方案、独立 Worker/Scheduler、消息代理、对象存储、OIDC、真实测试环境 Provider 联调，以及多实例部署/恢复演练。
