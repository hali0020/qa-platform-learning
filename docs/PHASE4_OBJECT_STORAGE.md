...
阶段六 B 的 Provider Run Artifact 已把这组原则落成一个具体业务示例：先写
`pending`，对象保存与 SHA-256/大小确认后转 `ready`，失败转 `failed` 并补偿清理；
删除先 quarantine，数据库提交失败则 restore，并记录审计。这里不应扩写为“所有
通用附件都已有后台 finalize/outbox/孤儿回收”，后者仍是对象存储深化项。
